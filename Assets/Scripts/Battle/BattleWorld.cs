    ...
    public sealed partial class BattleWorld
    {
        ...
        void Spawn(SpawnSpec spec)
        {
            for (int i = 0; i < spec.Count; i++)
            {
                ...
                var at = new Vector2(
                    FieldLayout.ColumnX(col) + UnityEngine.Random.Range(-0.08f, 0.08f),
                    GameConstants.SpawnY + i * 0.18f);
        ...
        EnemyActor Make(EnemyId id, Vector2 at)
        {
        ...
        void SpawnSplit(EnemyActor from, int count)
        {
            ...
            for (int i = 0; i < count; i++)
            {
                ...
                Enemies.Add(Make(EnemyId.Swarm,
                    new Vector2(Mathf.Clamp(from.Pos.x + ox, min, max), from.Pos.y)));
            }
        ...
        void ResolveHit(EnemyActor e, ShotMods m, BulletActor src, HitSource from, float flat = -1f)
        {
            ...
            ShowFloat(e.Pos + Vector2.up * 0.22f, n.ToString(), ink, heavy ? 1.42f : 1f);
            if (e.Hp <= 0f) Kill(e, m);
        }
        ...
        void PourLeech(float amount, float cap)
        {
                   ...
                   && (cap <= 0f || _leechGiven < cap))
            {
                ...
                _leechGiven += 1f;
                BaseHp++;
                ShowFloat(new Vector2(0f, GameConstants.EmitterY + 0.7f), "+1", InkTheme.Wood, 1.15f);
            }
        }
        ...
        void Kill(EnemyActor e, ShotMods m)
        {
            e.Dead = true;
            Gold += e.Gold;
            Energy = Mathf.Min(GameConstants.EnergyMax, Energy + e.Gold * GameConstants.EnergyPerGold);
                ...
                if (e.HardTime > 0f)
                {
                    ...
                    if (e.HardTime <= 0f)
                    {
                        ...
                        if (wasStun && e.Shock > 0f)
                        {
                            e.Hp -= e.Shock;
                            e.HitFlash = 0.16f;
                            ShowFloat(e.Pos + Vector2.up * 0.24f,
                                Mathf.Max(1, Mathf.RoundToInt(e.Shock)).ToString(),
                                InkTheme.ThunderHi, 1.1f);
                            e.Shock = 0f;
                            if (e.Hp <= 0f) { Kill(e, null); continue; }
                        }
                    }
        ...
        void TickConfused(EnemyActor e, float dt)
        {
            ...
            best.Hp -= dmg;
            best.HitFlash = 0.14f;
            ShowFloat(best.Pos + Vector2.up * 0.2f,
                Mathf.Max(1, Mathf.RoundToInt(dmg)).ToString(), InkTheme.Confuse, 0.9f);
            Bursts.Add(new FxBurst
            {
